...
//    private $logger;
     ...
     * @throws \Exception
     */
    public function __construct($host, $consumer, $consumer_secret){
        // check the host name
        if (!$this->isValidHost($host)){
            throw new \Exception("Invalid host: '{$host}'");
        }
        // set the consumer, token & signature method
        $this->consumer = new oAuthConsumer($consumer, $consumer_secret);
//        $this->accessToken = $accessToken;
        $this->accessToken = new oAuthToken(null, null);
